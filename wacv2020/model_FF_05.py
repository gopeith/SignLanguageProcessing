...
dim_y = 98 # ouput size
dim_emb = 64 # embeddings size
dim_rep = 1024 # "repository" size
dim_h = 512 # hidden layers size

Nh = 5 # number of modules in the translator
...
def get_dense(Lname, dim, stop, activation=lambda x:x, use_bias=True, init_weights=None):
  ...
  if not Lname in Ls:
    ...
    Ls[Lname] = Dense2(dim, activation=activation, use_bias=use_bias, weights=weights)
  L = Ls[Lname]
  L.stop = stop
  return L
...
def invEmbB(x, stop=False, mod=""):
  ...
  x = ConcatenateMatrixAndVectors()([x] + addinf)
  L = get_dense("An_b" + mod, dim_rep, stop, activation=keras.activations.tanh, use_bias=True)
  x = L(x)
  L = get_dense("An_c" + mod, dim_y, stop, use_bias=True)
  x = L(x)
...
text, dim_text = loadText("data/demo/text/text")

hfTar = h5py.File("data/demo/keypoints/keypoints-04-deltas-norm.h5", "r")

info_spk, dim_spk = loadInfo("data/demo/info/spk", dtype=dtype)
...
# mean of outputs 
mu_values = loadMatrixFloatText("temp/demo/mu", dtype=dtype)
...
ybes.append(ae)
for ih in range(Nh):
  ybe = translation_from_A_to_B(ybe, Nh=Nh, mod="%d" % ih)
  ybes.append(ybe)

# outputs
ys = []
for i in range(len(ybes)):
  ybe = ybes[i]
  y = invEmbB(ybe, False, mod="%d" % i)
  ys.append(y)
ys.reverse()
